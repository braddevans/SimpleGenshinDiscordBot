...
public class TableRenderer {
	...
	private int[] getCollumnWidths(String[][] table) {
		int[] collums = new int[width];
		for (String[] strings : table)
			for (int hIndex = 0; hIndex < width; hIndex++)
				if (strings[hIndex].length() + 1 > collums[hIndex])
					collums[hIndex] = strings[hIndex].length() + 1;
		collums[collums.length-1] -= 1;
		return collums;
	}
	...
	public String build() {
		...
		int[] widths = getCollumnWidths(table);
